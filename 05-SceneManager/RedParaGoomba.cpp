...
void CRedParaGoomba::OnCollisionWithKoopa(LPCOLLISIONEVENT e) {
	if (e->obj->GetState() == KOOPA_STATE_SHELL_MOVE) {
		if (isWinged) {
			isWinged = false;
		}
		else {
...
void CRedParaGoomba::OnCollisionWithPlatform(LPCOLLISIONEVENT e) {
	...
	if (e->ny < 0) {
		isOnPlatform = true;
		float platformY = platform->GetPositionY();
		y = platformY - RED_PARA_GOOMBA_BBOX_HEIGHT + 1;
	}
}
